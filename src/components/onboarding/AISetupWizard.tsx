  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-ff-text-primary">
            Welcome to AI-Powered FlashFusion
          </h2>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {useCase.popular && <Badge className="ff-btn-accent text-xs">Popular</Badge>}
                  </div>
...
const ExperienceStep: React.FC<WizardStepProps> = ({
  onNext,
  onPrev,
  stepData,
  updateStepData,
  ...
  return (
    <div className="space-y-6">
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4">
                  ...
                  >
                    <ProviderIcon className="w-6 h-6" style={{ color: provider.color }} />
                  </div>
                ...
                <div className="ml-4">
                  {isSelected && <CheckCircle2 className="w-6 h-6 text-ff-primary" />}
                </div>
              </div>
            </motion.button>
          );
      ...
      <Alert>
        ...
        <AlertDescription>
          You'll need to provide API keys for selected providers in the next step. Don't worry -
          we'll guide you through getting them!
        </AlertDescription>
      </Alert>
...
const APIKeyStep: React.FC<WizardStepProps> = ({ onNext, onPrev, stepData, updateStepData }) => {
  ...
  const saveAndTestKey = async (provider: AIProvider) => {
    ...
    try {
      ...
      toast.success(
        `${providerInfo[provider as keyof typeof providerInfo]?.name} API key verified!`
      );
    } catch (error) {
      console.error('API key test failed:', error);
      toast.error(
        `Failed to verify ${provider} API key: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-ff-text-secondary mt-2">Add your API keys to start using AI features</p>
      </div>
      ...
      <div className="space-y-4">
        {selectedProviders.map((provider: AIProvider) => {
          ...
          return (
            <Card
              key={provider}
              className={`${isCompleted ? 'border-ff-success bg-ff-success/5' : ''}`}
            >
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {isCompleted && <CheckCircle2 className="w-6 h-6 text-ff-success" />}
                </div>
...
const CompleteStep: React.FC<WizardStepProps> = ({ onComplete, stepData }) => {
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <Button onClick={() => onComplete()} className="ff-btn-primary ff-hover-glow" size="lg">
