...
export function ExternalAppIntegrationHub() {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<keyof typeof SUPPORTED_PLATFORMS | null>(
    null
  );
  ...
  const filteredApps = connectedApps.filter((app) => {
    ...
    const matchesCategory =
      selectedCategory === 'all' || SUPPORTED_PLATFORMS[app.platform].category === selectedCategory;
  ...
  const handleSyncApp = async (appId: string) => {
    setConnectedApps((prev) =>
      prev.map((app) =>
        app.id === appId ? { ...app, status: 'connecting' as IntegrationStatus } : app
      )
    );
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="ff-text-display text-4xl mb-2">External App Integration Hub</h1>
          ...
          <Button onClick={() => setShowConnectionDialog(true)} className="ff-btn-primary">
          ...
          <Card className="ff-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <ProfessionalIcon icon={Network} size="lg" variant="functional" context="primary" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="ff-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <ProfessionalIcon icon={Sync} size="lg" variant="functional" context="success" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="ff-card">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="ff-text-title text-2xl font-bold">
                    {connectedApps.length > 0
                      ? Math.round(
                          connectedApps.reduce((sum, app) => sum + app.analytics.uptime, 0) /
                ...
                <ProfessionalIcon icon={Activity} size="lg" variant="functional" context="accent" />
              </div>
            </CardContent>
          </Card>
        </div>
                ...
                return (
                  <Card key={app.id} className="ff-card-interactive">
                    ...
                    <CardContent className="space-y-4">
                      {app.description && (
                        <p className="ff-text-caption text-muted-foreground">{app.description}</p>
                      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(SUPPORTED_PLATFORMS).map(([key, platform]) => (
                <Card key={key} className="ff-card-interactive">
                  ...
                  <CardContent className="space-y-4">
                    <p className="ff-text-caption text-muted-foreground">{platform.description}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input id="app-name" placeholder={`My ${platformInfo.name} App`} />
        </div>

        <div>
          <Label htmlFor="app-url">App URL</Label>
          <Input id="app-url" placeholder={`https://my-app.${platform}`} />
        </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          This will establish a {connectionMethod} connection with {authType} authentication. You
          can modify these settings later in the app management panel.
        </AlertDescription>
      </Alert>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button onClick={() => onConnect(platform)} disabled={isLoading}>
          {isLoading ? (
            <>
