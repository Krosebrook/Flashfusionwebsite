    ...
    .sort((a, b) => {
      ...
      const scoreA =
        (a.personalization.relevanceScore +
          a.personalization.userBehaviorMatch +
          a.personalization.completionLikelihood) /
        3;
      const scoreB =
        (b.personalization.relevanceScore +
          b.personalization.userBehaviorMatch +
          b.personalization.completionLikelihood) /
        3;
...
const calculateRelevanceScore = (rec: any, context: UserContext): number => {
  ...
  if (
    context.workingOn.some((work) => rec.description.toLowerCase().includes(work.toLowerCase()))
  ) {
    score += 20;
  }
...
const RecommendationCard = ({
  ...
  const overallScore = useMemo(() => {
    const { relevanceScore, userBehaviorMatch, completionLikelihood } =
  ...
  return (
    ...
    >
      <Card
        className={`ff-card-interactive h-full transition-all duration-300 ${
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div>
                  ...
                  <div className="font-medium">
                    {Math.round(recommendation.personalization.relevanceScore)}%
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Behavior</div>
                  <div className="font-medium">
                    {Math.round(recommendation.personalization.userBehaviorMatch)}%
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Completion</div>
                  <div className="font-medium">
                    {Math.round(recommendation.personalization.completionLikelihood)}%
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              No new recommendations at the moment. Keep using FlashFusion to get personalized
              suggestions.
            </p>
          </div>
          <Button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div className="space-y-1">
              <div className="text-2xl font-bold text-secondary">
                {Math.round(
                  visibleRecommendations.reduce((acc, rec) => acc + rec.confidence, 0) /
              ...
              <div className="text-2xl font-bold text-green-500">
                {
                  visibleRecommendations.filter(
                    (rec) => rec.estimatedTime && parseInt(rec.estimatedTime) <= 30
                  ).length
                }
              </div>
