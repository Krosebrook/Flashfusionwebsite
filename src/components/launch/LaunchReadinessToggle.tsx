...
export function LaunchReadinessToggle({
  ...
  const performReadinessChecks = useCallback(async () => {
    ...
    try {
        ...
        readinessChecks.map(async (check) => {
          ...
          switch (check.id) {
            case 'production-deployment':
              ...
              details =
                status === 'warning'
                  ? 'Running on localhost - deploy to production'
            ...
            case 'api-endpoints':
              ...
              details =
                status === 'passed' ? 'All endpoints responding' : 'Some endpoints not responding';
              break;
            ...
            case 'security-scan':
              ...
              details =
                status === 'passed'
                  ...
                  : 'Minor security recommendations available';
              break;
      ...
      if (isReady) {
        ...
        toast.error(
          `${failedRequired.length} critical issue(s) need to be resolved before launch.`
        );
      } else {
  ...
  const handleLaunchReadinessToggle = useCallback(
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              {readinessState.overallScore > 0 && (
                <Badge variant="outline">{Math.round(readinessState.overallScore)}% Ready</Badge>
              )}
            </div>
        ...
        {readinessState.overallScore > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold">{Math.round(readinessState.overallScore)}%</span>
            </div>
            <Progress value={readinessState.overallScore} className="h-2 ff-progress-bar" />
          </div>
        )}
          ...
          <div className="space-y-3">
            {readinessState.blockers.length > 0 && (
              <Alert className="border-destructive bg-destructive/5">
                ...
                <AlertDescription>
                  {readinessState.blockers.length} issue(s) must be resolved:{' '}
                  {readinessState.blockers.join(', ')}
                </AlertDescription>
              </Alert>
            )}
              ...
              {readinessChecks.map((check) => (
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground font-inter">{check.description}</p>
