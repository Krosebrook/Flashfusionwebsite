...
export function LaunchDayPreparationChecklist() {
  ...
  const handleItemStatusChange = useCallback(
    (itemId: string, newStatus: ChecklistItem['status']) => {
  ...
  const handleAutoExecuteItem = useCallback(
    async (itemId: string) => {
      const item = checklistItems.find((i) => i.id === itemId);
      if (!item || !item.automation_available) return;

      setChecklistItems((prev) =>
        prev.map((i) => (i.id === itemId ? { ...i, status: 'in_progress' } : i))
      );
      ...
      try {
        ...
        setChecklistItems((prev) =>
          prev.map((i) =>
            i.id === itemId ? { ...i, status: 'completed', completion_time: Date.now() } : i
          )
        );

        toast.success(`Automated execution of "${item.title}" completed!`);
        analyticsService.trackFeatureUsage('automated-task-completed', {
          itemId,
  ...
  const handleAutoExecuteAll = useCallback(async () => {
    ...
    try {
      const automatedItems = checklistItems.filter(
        (item) =>
          item.automation_available && (item.status === 'pending' || item.status === 'failed')
      );

      for (const item of automatedItems) {
        // Check dependencies
        const dependenciesCompleted = item.dependencies.every(
          (depId) => checklistItems.find((i) => i.id === depId)?.status === 'completed'
        );
    ...
    <div className="space-y-6 ff-fade-in-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge
            variant={
              launchMetrics.risk_level === 'low'
                ? 'default'
                : launchMetrics.risk_level === 'medium'
                  ? 'secondary'
                  : 'destructive'
            }
      ...
      {launchMetrics.critical_items_remaining > 0 && (
        <Alert className="border-destructive bg-destructive/5">
          ...
          <AlertDescription>
            {launchMetrics.critical_items_remaining} critical item
            {launchMetrics.critical_items_remaining === 1 ? '' : 's'} must be completed before
            launch. Estimated time remaining:{' '}
            {Math.ceil(launchMetrics.estimated_time_remaining / 60)} hours.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-2">
              <p className="text-2xl font-bold">
                {launchMetrics.completion_percentage.toFixed(0)}%
              </p>
        ...
        <Card className="ff-card-interactive">
          <CardContent className="p-6">
            ...
            <p
              className={`text-2xl font-bold ${launchMetrics.critical_items_remaining === 0 ? 'text-green-500' : 'text-red-500'}`}
            >
              {launchMetrics.critical_items_remaining}
            </p>
            <p className="text-sm text-muted-foreground">remaining to complete</p>
          </CardContent>
        </Card>
        ...
        <Card className="ff-card-interactive">
          <CardContent className="p-6">
            ...
            <p
              className={`text-2xl font-bold ${launchMetrics.team_availability >= 90 ? 'text-green-500' : launchMetrics.team_availability >= 75 ? 'text-yellow-500' : 'text-red-500'}`}
            >
        ...
        <Card className="ff-card-interactive">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-muted-foreground">tasks can be automated</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="checklist" className="space-y-4">
        ...
        <TabsContent value="checklist" className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {['technical', 'marketing', 'content', 'legal', 'team', 'monitoring'].map(
              (category) => {
          ...
          <div className="space-y-3">
            {filteredItems.map((item) => {
              const dependencyItems = item.dependencies
                .map((depId) => checklistItems.find((i) => i.id === depId))
                .filter(Boolean);
              const dependenciesCompleted = item.dependencies.every(
                (depId) => checklistItems.find((i) => i.id === depId)?.status === 'completed'
              );
              const assignee = teamMembers.find((member) =>
                member.assigned_items.includes(item.id)
              );
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p
                              className={`font-medium ${item.automation_available ? 'text-green-500' : 'text-muted-foreground'}`}
                            >
          ...
          <div className="space-y-4">
            {launchPhases.map((phase) => {
              const phaseItems = phase.items
                .map((itemId) => checklistItems.find((item) => item.id === itemId))
                .filter(Boolean);
        ...
        <TabsContent value="team" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {teamMembers.map((member) => {
              const assignedItems = member.assigned_items
                .map((itemId) => checklistItems.find((item) => item.id === itemId))
                .filter(Boolean);
              const completedItems = assignedItems.filter((item) => item.status === 'completed');
              const progress =
                assignedItems.length > 0 ? (completedItems.length / assignedItems.length) * 100 : 0;
                ...
                >
                  <CardContent className="p-6">
                    <div className="space-y-4">
                      ...
                      <Button size="sm" variant="outline" className="ff-focus-ring w-full">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div>
                                  ...
                                  <p className="font-medium">
                                    {new Date(phase.start_time).toLocaleTimeString()}
                                  </p>
                                </div>
                                <div>
