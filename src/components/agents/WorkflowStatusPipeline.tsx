...
import { CheckCircle, Clock, PlayCircle, ArrowRight, Zap, AlertCircle, Pause } from 'lucide-react';
      ...
      <div className="relative">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <React.Fragment key={step.id}>
                ...
                >
                  <div className="relative cursor-pointer" onClick={() => onStepClick?.(step.id)}>
                    {/* Agent Avatar */}
                    <motion.div
                    ...
                    >
                      ...
                      <StepIcon className="h-6 w-6 text-white relative z-10" />
                      ...
                      {step.status === 'active' && step.progress !== undefined && (
                        <svg className="absolute inset-0 w-full h-full -rotate-90">
                          ...
                          <motion.circle
                            ...
                            animate={{
                              strokeDashoffset:
                                2 * Math.PI * 30 - 2 * Math.PI * 30 * (step.progress / 100),
                            }}
                    ...
                    <div className="mt-3 text-center min-w-[120px]">
                      <div className="font-semibold text-sm text-gray-900">{step.agentName}</div>
                      <div className="text-xs text-gray-500 mt-1">{step.taskName}</div>
        ...
        <AnimatePresence>
          {steps.some((s) => s.status === 'active' || s.status === 'error') && (
            ...
            >
              {steps
                .filter((s) => s.status === 'active' || s.status === 'error')
                .map((step) => (
