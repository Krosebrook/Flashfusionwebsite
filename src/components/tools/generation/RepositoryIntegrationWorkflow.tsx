...
export function EnhancedComponent({ title, onAction }: EnhancedComponentProps) {
  ...
  const getStatusIcon = (status: WorkflowStep['status']) => {
    switch (status) {
      case 'completed':
        return <CheckCircle2 className="w-4 h-4 text-ff-success" />;
      case 'running':
        return <RefreshCw className="w-4 h-4 text-ff-primary animate-spin" />;
      case 'error':
        return <AlertCircle className="w-4 h-4 text-ff-error" />;
      default:
        return null;
    }
  };
  ...
  const getStatusColor = (status: WorkflowStep['status']) => {
    switch (status) {
      case 'completed':
        return 'border-ff-success bg-ff-success/10';
      case 'running':
        return 'border-ff-primary bg-ff-primary/10';
      case 'error':
        return 'border-ff-error bg-ff-error/10';
      default:
        return 'border-ff-text-muted bg-ff-surface';
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="workflow" className="space-y-6">
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="text-sm text-ff-text-muted">
                    {isRunning
                      ? `Running step ${currentStep + 1} of ${workflowSteps.length}`
                      : 'Workflow completed'}
                  </div>
                </div>
              </CardContent>
          ...
          <div className="grid gap-4">
            {workflowSteps.map((step, index) => (
              <Card
                key={step.id}
                className={`ff-card-interactive transition-all ${getStatusColor(step.status)}`}
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-lg font-medium text-ff-text-primary">{index + 1}</div>
                      </div>
        ...
        <TabsContent value="analyzer">
          {repository ? (
            <RepositoryAnalyzer repository={repository} onAnalysisComplete={setAnalysisResults} />
          ) : (
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>Please connect a repository to use the analyzer.</AlertDescription>
            </Alert>
          )}
        </TabsContent>
        ...
        <TabsContent value="generator">
          {repository ? (
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>Please connect a repository to use the generator.</AlertDescription>
            </Alert>
          )}
        </TabsContent>
