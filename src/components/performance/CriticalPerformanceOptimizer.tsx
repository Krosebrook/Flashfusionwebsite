...
const CriticalPerformanceOptimizer = memo(
  ({ children, onMetricsUpdate }: CriticalPerformanceOptimizerProps) => {
    ...
    const monitorMemoryUsage = useCallback(() => {
      ...
      if (memoryInfo) {
        ...
        if (memoryInfo.percentage > 90) {
          ...
          if (now - lastMemoryAlert > 10000) {
            // Only alert once every 10 seconds
    ...
    const countComponents = useCallback(() => {
      ...
      const reactElements = Array.from(allElements).filter(
        (el) =>
      ...
      const checkPerformanceBudget = () => {
        ...
        Object.entries(budgets).forEach(([metric, threshold]) => {
          ...
          if (typeof value === 'number' && value > threshold) {
            console.warn(
              `⚠️ Performance budget exceeded for ${metric}:`,
              value,
              'threshold:',
              threshold
            );
    ...
    useEffect(() => {
      ...
      if (renderTime > 16) {
        ...
        if (now - lastSlowRenderTime > 5000) {
          // Only track once every 5 seconds
...
export const usePerformanceMonitoring = (componentName: string) => {
  ...
  const trackAsyncOperation = useCallback(
    (operationName: string, operation: () => Promise<any>) => {
      try {
        if (performanceMonitor.current) {
          return performanceMonitor.current.measureAsyncOperation(
