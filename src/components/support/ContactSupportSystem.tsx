...
export function ContactSupportSystem({
  ...
  const handleInputChange = useCallback(
    (field: keyof SupportTicket, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        submit: 'Failed to submit ticket. Please try again or use alternative contact methods.',
      });
    } finally {
            ...
            <div className="bg-[var(--ff-primary)]/10 border border-[var(--ff-primary)]/20 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">
                    {categories.find((c) => c.value === formData.category)?.responseTime}
                  </span>
                </div>
              </div>
            </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {categories.map((category) => (
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      <div
                        className={`mt-1 ${formData.category === category.value ? 'text-[var(--ff-primary)]' : 'text-[var(--ff-text-muted)]'}`}
                      >
                        {category.icon}
                      </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-lg p-4">
              ...
              <div className="space-y-2 text-xs text-[var(--ff-text-muted)]">
                <p>
                  • Check our <span className="text-[var(--ff-primary)]">Knowledge Base</span> for
                  common solutions
                </p>
                <p>
                  • Join our <span className="text-[var(--ff-secondary)]">Community Discord</span>{' '}
                  for peer support
                </p>
