...
const foundationTasks = [
  {
    ...
    description:
      'Establish clear user volume and MRR growth targets for system scalability planning',
    completed: false,
    priority: 'critical',
  ...
  },
  {
    ...
    description:
      'Define and document SOC 2, GDPR, HIPAA compliance requirements and implementation plan',
    completed: false,
    priority: 'critical',
  ...
  },
  {
    ...
    description:
      'Define clear team structure, roles, responsibilities, and communication protocols',
    completed: false,
    priority: 'high',
...
const qualityGateTasks = [
  {
    ...
    description:
      'Build comprehensive automated testing pipeline with unit, integration, and e2e tests',
    status: 'todo',
    priority: 'critical',
...
export function ImmediateNextActions({}: ImmediateNextActionsProps) {
  ...
  const priorityProgress = useMemo(() => {
    const priority1Progress =
      (foundationTasksState.filter((t) => t.completed).length / foundationTasksState.length) * 100;
    const priority2Progress =
      (qualityTasksState.filter((t) => t.status === 'done').length / qualityTasksState.length) *
      100;
    const priority3Progress =
      metricsTasksState.reduce((sum, task) => sum + task.progress, 0) / metricsTasksState.length;
              ...
              {[1, 2, 3].map((priority) => (
                <Card
                  ...
                  className={`ff-card cursor-pointer transition-all duration-200 ${
                    selectedPriority === priority ? 'border-2' : 'hover:border-opacity-50'
                  }`}
                  style={{
                    borderColor:
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {priority === 1
                            ? priorityProgress.priority1
                            : priority === 2
                              ? priorityProgress.priority2
        ...
        <Card className="ff-card">
          <CardContent className="p-0">
            <Tabs
              ...
              onValueChange={(value) =>
                setSelectedPriority(parseInt(value.replace('priority', '')) as 1 | 2 | 3)
              }
              className="w-full"
            >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ff-text-sm text-[var(--ff-text-muted)] mt-1">
                      Critical foundation setup for scalability, compliance, team structure, and
                      monitoring
                    </p>
                  </div>
                  <Badge className="ff-badge-error">
                    {foundationTasksState.filter((t) => t.completed).length}/
                    {foundationTasksState.length} Complete
                  </Badge>
                </div>
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-2">
                              <h4
                                className={`ff-text-sm ${task.completed ? 'line-through text-[var(--ff-text-muted)]' : 'text-[var(--ff-text-primary)]'}`}
                                style={{ fontWeight: 'var(--ff-weight-semibold)' }}
                              >
                                {task.title}
                              </h4>
                            ...
                            <div className="grid md:grid-cols-3 gap-4 mb-4 text-xs">
                              <div>
                                ...
                                <span
                                  className="text-[var(--ff-text-primary)] ml-1"
                                  style={{ fontWeight: 'var(--ff-weight-semibold)' }}
                                >
                                  {task.assignee}
                                </span>
                              ...
                              <div>
                                ...
                                <span
                                  className="text-[var(--ff-text-primary)] ml-1 capitalize"
                                  style={{ fontWeight: 'var(--ff-weight-semibold)' }}
                                >
              ...
              <TabsContent value="priority2" className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ff-text-sm text-[var(--ff-text-muted)] mt-1">
                      Implement comprehensive quality gates, testing frameworks, and deployment
                      processes
                    </p>
                  </div>
                  <Badge className="ff-badge-warning">
                    {qualityTasksState.filter((t) => t.status === 'done').length}/
                    {qualityTasksState.length} Complete
                  </Badge>
                </div>
                  ...
                  {['todo', 'in-progress', 'done'].map((status) => (
                    <div key={status} className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge
                          className={`ff-badge-${status === 'todo' ? 'secondary' : status === 'in-progress' ? 'warning' : 'success'} text-xs`}
                        >
                      ...
                      <div className="space-y-3 min-h-[400px] p-3 bg-[var(--ff-surface)] rounded-lg">
                        {qualityTasksState
                          ...
                          .map((task) => (
                            <Card
                              key={task.id}
                              className="ff-card cursor-pointer hover:border-[var(--ff-warning)]/30"
                            >
                              <CardContent className="p-3">
                                ...
                                <p className="ff-text-xs text-[var(--ff-text-muted)] mb-3">
                                  {task.description}
                                </p>
              ...
              <TabsContent value="priority3" className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="ff-text-sm text-[var(--ff-text-muted)] mt-1">
                      Analytics setup, business metrics dashboard, user feedback systems, and A/B
                      testing framework
                    </p>
                  </div>
                  <Badge className="ff-badge-success">
                    {Math.round(
                      metricsTasksState.reduce((sum, task) => sum + task.progress, 0) /
                        metricsTasksState.length
                    )}
                    % Complete
                  </Badge>
                </div>
                        ...
                        <div className="space-y-4">
                          {task.components.map((component, index) => (
                            <div key={index} className="p-4 bg-[var(--ff-surface)] rounded-lg">
                                  ...
                                  onCheckedChange={(checked) => {
                                    ...
                                    const newComponents = task.components.map((c) =>
                                      c.name === component.name
                                        ? { ...c, completed: checked as boolean }
                                        : c
                                    );
                                    const newProgress = Math.round(
                                      (newComponents.filter((c) => c.completed).length /
                              ...
                              <div>
                                <h6
                                  className="ff-text-xs text-[var(--ff-text-primary)] mb-2"
                                  style={{ fontWeight: 'var(--ff-weight-semibold)' }}
                                >
