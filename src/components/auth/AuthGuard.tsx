 ...
 */
export function AuthGuard({ children, fallback, requiredPermission, redirectTo }: AuthGuardProps) {
  const auth = useAuth();
  ...
  useEffect(() => {
    ...
    if (routeLevel === 'protected' && !auth.isAuthenticated) {
      const redirectUrl =
        redirectTo ||
        `/auth?mode=signin&redirect=${encodeURIComponent(window.location.pathname + window.location.search)}`;
