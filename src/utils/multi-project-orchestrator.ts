import { Brain, Users, Layers, Settings, Share2 } from 'lucide-react';
import {
...
export const getPriorityColor = (priority: string) => {
  return (
    PRIORITY_COLORS[priority as keyof typeof PRIORITY_COLORS] || 'border-gray-500 text-gray-700'
  );
};

export const getImplementationEffortColor = (effort: string) => {
  return (
    IMPLEMENTATION_EFFORT_COLORS[effort as keyof typeof IMPLEMENTATION_EFFORT_COLORS] ||
...
export const getBottleneckRiskColor = (risk: string) => {
  return (
    BOTTLENECK_RISK_COLORS[risk as keyof typeof BOTTLENECK_RISK_COLORS] ||
