...
export class GamificationService {
  ...
  static async awardXP(userId: string, amount: number, reason: string): Promise<boolean> {
    try {
      ...
      const { data: leveledUp } = await supabase.rpc('check_level_up', { user_uuid: userId });
  ...
  static async checkAchievements(userId: string): Promise<Achievement[]> {
    try {
      ...
      const [toolUsageData, likesData, platformsData] = await Promise.all([
        supabase.from('ai_tool_usage').select('tool_id').eq('user_id', userId),
        supabase
          .from('project_likes')
          .select('project_id')
          .in('project_id', supabase.from('projects').select('id').eq('user_id', userId)),
        supabase
          .from('deployments')
          .select('platform')
          .eq('user_id', userId)
  ...
  static async getUserRank(userId: string): Promise<number | null> {
    try {
      const { data } = await supabase.from('leaderboard').select('rank').eq('id', userId).single();
