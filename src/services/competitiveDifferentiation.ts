...
export class CompetitiveDifferentiationService {
  ...
  private static async analyzeCode(code: string): Promise<CodeReview['analysis']> {
    ...
    return {
      quality: Math.min(
        100,
        60 + (hasComments ? 15 : 0) + (hasTypeScript ? 15 : 0) + (hasTests ? 10 : 0)
      ),
  ...
  private static async awardXPForCodeReview(
    userId: string,
    analysis: CodeReview['analysis']
  ): Promise<void> {
    const baseXP = 100;
  ...
  static async submitChallengeEntry(
    challengeId: string,
    userId: string,
    projectId: string
  ): Promise<void> {
  ...
  private static adaptLearningPath(currentPath: any, analysis: any): LearningPath {
    ...
    const nextChallenges = this.suggestChallenges(
      currentPath?.currentLevel || 'beginner',
      strengths,
      weaknesses
    );
  ...
  private static categorizeTools(toolUsage: any[]): any {
    ...
    Object.keys(categories).forEach((category) => {
      categories[category].successRate =
        categories[category].successes / categories[category].count;
    });
  ...
  private static suggestChallenges(
    level: string,
    strengths: string[],
    weaknesses: string[]
  ): string[] {
  ...
  private static determineLevel(analysis: any): 'beginner' | 'intermediate' | 'advanced' {
    ...
    const successRate =
      analysis.toolUsage.filter((u: any) => u.success).length / analysis.toolUsage.length;
  ...
  private static calculateProgress(analysis: any): { [skill: string]: number } {
    ...
    skills.forEach((skill) => {
      const categoryTools = analysis.toolUsage.filter(
        (u: any) => this.getToolCategory(u.tool_id) === skill
      );

      if (categoryTools.length > 0) {
        const successRate =
          categoryTools.filter((u: any) => u.success).length / categoryTools.length;
