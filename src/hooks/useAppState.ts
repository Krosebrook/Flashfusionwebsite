  ...
  const initializeApp = useCallback(async (attempt = 1): Promise<void> => {
    try {
      ...
      try {
        ...
        const isValidToken =
          authToken && authToken !== 'null' && authToken !== 'undefined' && authToken.length > 10;
      ...
      if (attempt > 1) {
        ...
        setTimeout(
          () => document.body.classList.remove('ff-scale-pop'),
      ...
      if (attempt < APP_CONFIG.RETRY_ATTEMPTS) {
        ...
        const delay =
          Math.pow(2, attempt) * APP_CONFIG.ANIMATION_DELAYS.RETRY_BASE + Math.random() * 500;
        setTimeout(() => {
